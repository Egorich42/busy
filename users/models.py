...
def curent_finace_states(start, end, cursor,nalog_system):
    ...
    full_list_ishod = transform_sql_to_list(cursor, sq_c.sel_docs_with_ishod_nds, sq_c.tn_providers,  "'"+start+"'",  "'"+str(end)+"'" )
    list_vhod_tn = transform_sql_to_list(cursor, sq_c.sel_vhod_tn_nds, sq_c.tn_buyers,  "'"+start+"'",  "'"+str(end)+"'" )
    list_vhod_usl = transform_sql_to_list(cursor, sq_c.sel_vhod_usl_nds, sq_c.tn_buyers,  "'"+start+"'",  "'"+str(end)+"'" )



    nds_polucheny = round(sum([i['summ'] for i in from_providers])/6,2)
    nds_otpravleny = round(sum([i['summ'] for i in to_buyers])/6,2)

    usn = str(round(sum([i['summ'] for i in to_buyers])*0.05,2))
...
#    testo=[]
#    for i in from_providers:
#        testo+=[{i['document_name'],i['contragent_name'],round(i['summ']/6,2)}]


#    print(nds_polucheny,nds_otpravleny)
